...
def get_appointments(request: Request, client_id: int = None, appointment_id: int = None, before: int = None, after: str = None, paid: bool = False, db: Session = Depends(get_db), auth_token: schemas.TokenData = Depends(oauth2.verify_access_token)):
    ...
    if utils.is_set(appointment_id):
        if utils.is_set(before) or utils.is_set(after) or utils.is_set(paid):
            raise HTTPException(status.HTTP_400_BAD_REQUEST, 'Cannot set constraints when fetching by id')
        appointment = db.query(models.Appointment).filter(models.Appointment.id == appointment_id).first()
        if not appointment or appointment.client_id != client_id:
            raise HTTPException(status.HTTP_404_NOT_FOUND, f'Cannot find appointment with id: {appointment_id}')
        return appointment
    else:
        appointments = db.query(models.Appointment).filter(models.Appointment.client_id == client_id)
...
def make_appointment(request: Request, appointment_data: schemas.POSTAppointmentInput, db: Session = Depends(get_db), auth_token: schemas.TokenData = Depends(oauth2.verify_access_token)):
    ...
    client = db.query(models.Client).filter(models.Client.id == appointment_data.client_id).first()
    if not client:
        raise HTTPException(status.HTTP_404_NOT_FOUND, f'Client with id: {appointment_data.client_id} does not exists')
    ...
    appointment = db.query(models.Appointment).filter(models.Appointment.client_id == appointment_data.client_id).filter(models.Appointment.date == _data['date']).first()
    if appointment:
