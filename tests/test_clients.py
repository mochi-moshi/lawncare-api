...
@pytest.mark.parametrize(
    "element", ("name", "email", "address", "password", "phone_number")
)
def test_client_post_data_non_null(
    client: TestClient,
...
def test_client_post_data_constraints(
    client: TestClient,
    session: TestSessionLocal,
    element: str,
    sample_client_data: dict,
...
def test_client_create(
    client: TestClient,
    session: TestSessionLocal,
    sample_client_POST: schemas.POSTClientInput,
):
    response = client.post("/client", json=sample_client_POST.dict())
...
def test_client_create_duplicate(
    client: TestClient, session: TestSessionLocal, sample_client_data: dict
):
    add_client(sample_client_data, session)
    response = client.post("/client", json=sample_client_data)
...
def test_client_login_does_not_exist(
    client: TestClient, session: TestSessionLocal, sample_client_data: dict
):
    response = client.post(
        "/auth/login",
    ...
    token = create_access_token(
        {
            ...
            "host": utils.hash("localhost", 5432),
            "testing": "True",
        }
    )
...
def test_client_delete(
    client: TestClient, session: TestSessionLocal, sample_client: models.Client
):
    token = create_access_token(
        {
...
def test_admin_get_client(
    client: TestClient, session: TestSessionLocal, sample_client_data: dict
):
    new_client = add_client(sample_client_data, session)
    token = create_access_token(
        {"client_id": 0, "host": utils.hash("localhost", 5432), "testing": "True"}
    )
    client.headers = {**client.headers, "Authorization": f"Bearer {token}"}
    response = client.get("/client", params={"id": new_client.id})
    assert response.status_code == status.HTTP_200_OK
    json = response.json()
    assert json
    assert schemas.ClientPublic(**json) == schemas.ClientPublic(**sample_client_data)
...
def test_admin_delete_client(
    ...
    client.headers = {**client.headers, "Authorization": f"Bearer {token}"}
    response = client.delete("/client", params={"id": sample_client.id})
    assert response.status_code == status.HTTP_200_OK
    queried_client = session.query(models.Client).filter(models.Client.id == 1).first()
    assert not queried_client
